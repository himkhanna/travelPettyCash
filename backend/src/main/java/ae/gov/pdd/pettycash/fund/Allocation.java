...
/**
 * Admin → Trip-pool or Leader → Member allocation (CLAUDE.md §5).
 * {@code fromUserId == null} means "from the admin pool of {@link #sourceId}".
 */
@Entity
...
public class Allocation {
    ...
    /** Null = admin-pool allocation; non-null = peer (leader → member). */
    ...
    public void respond(FundsStatus status, Instant at) {
        if (this.status != FundsStatus.PENDING) {
            throw new IllegalStateException(
                "Cannot respond to allocation " + id + " — already " + this.status
            );
        }
